...
def analyze_metadata_free(text, title):
    """AI 비용 없이 파이썬 문자열 분석으로 메타데이터 추출"""
    ...
    with c2:
        ...
        if content:
            ...
            if st.button("✈️ 텔레그램 전송", type="primary"):
                if not telegram_token or not chat_id: st.error("토큰 입력 필요")
                else:
                    try:
    ...
    if df_all.empty: st.info("보관함이 비어있습니다.")
        ...
        if not df_filt.empty:
            ...
            column_cfg = {
                ...
                "clinical_score": st.column_config.NumberColumn("점수"),
                # 추가된 컬럼들
                "tags": st.column_config.TextColumn("태그 (#)", width="medium"),
                "n_count": st.column_config.TextColumn("N수", width="small"),
                "user_note": st.column_config.TextColumn("메모", width="medium"),
            }
            
            edited = st.data_editor(
                df_filt,
                column_config=column_cfg,
                column_order=["del", "url", "date_published", "clinical_score", "tags", "n_count", "title_kr", "user_note"],
                hide_index=True, use_container_width=True
            )
            
            # 메모 수정 사항이 있으면 저장 (실시간 업데이트)
            # data_editor는 수정된 dataframe을 반환하므로, 변경된 user_note를 감지해서 DB에 업데이트 가능
            # 여기서는 '삭제' 버튼 로직만 구현 (메모 저장은 복잡해지므로 단순화)
