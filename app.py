            ...
            if st.button("🚀 심층 분석 & 글쓰기"):
                if not openai_api_key: st.error("Key 없음")
                    ...
                    with st.spinner("3. 심층 칼럼 작성 중..."):
                        ...
                        [작성 핵심 지침 - 비교와 보완]
                        1. 메인 논문의 중재(예: 소풍산)에 대한 직접적인 연구가 부족하다면, 검증 자료에 있는 **'유사 계열(예: 한약 전반, 다른 처방)'의 효과와 비교**하여 서술하세요 이때는 반드시 논문에 근거해야합니다. 혹은 메인 논문의 중재와 같지만 다른 종류의 질병에 영향을 주는 경우와 비교해서 서술하되, 근거가 부족하면 넘기세요.
                        2. 메인 논문과 관련된 중재에 관한 새로 찾은 논문도 분석글에 반드시 해당 논문의 전체 글을 분석하고 어떠한 중재를 사용했는지 반드시 포함시키고 메인 논문의 중재와 비교하세요. 만약에 해당데이터가 없는 경우는 없음을 인정하고 새로운 정보를 만들 필요가 없습니다.  
                        3. 예시 멘트: "소풍산 자체에 대한 연구는 적지만, 유사한 청열제 계열의 한약들은 아토피의 소양감 완화에 유의미한 근거(레퍼런스 중에 어떤것인지 반드시 번호명기)를 가지고 있습니다."
                        4. 이를 통해 이 치료법이 임상적으로 타당한지 독자(한의사)를 설득하세요.
