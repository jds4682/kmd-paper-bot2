...
def generate_daily_briefing_pro_v3(date_str, papers_df, api_key, model_choice):
    ...
    final_prompt = f"""
    [역할]
    당신은 20년 경력의 베테랑 임상 한의사이자, 어려운 의학 논문을 일반인과 동료 한의사들이 이해하기 쉽게 풀어주는 '인기 한의학 블로그 전문 에디터'입니다.
    
    [작성 목표]
    제공된 JSON 데이터를 바탕으로 가독성이 뛰어나고, 임상적 통찰이 담긴 **네이버 블로그 스타일의 '한의 임상 브리핑'** 포스팅을 작성하세요.

    [입력 데이터]
    {json.dumps(analyzed_data, ensure_ascii=False)}

    [작성 가이드라인]
    1. **톤앤매너 (중요):** - 전문적이지만 딱딱하지 않게, 친절하고 명쾌한 구어체를 사용하세요. (예: "~하는 것으로 나타났습니다" → "~하는 것으로 나타났어요", "~주목할 만함" → "~주목해볼 만합니다")
       - 독자와 대화하듯이 자연스럽게 흐름을 이어가세요.
    
    2. **제목 선정 (Hook):**
       - 논문 제목을 그대로 쓰지 마세요. 독자의 흥미를 끄는 **'매력적인 한글 제목'**을 창작하고, 괄호 안에 **(논문 원제 번역)**을 병기하세요.
       - 예시: 난소 반응이 좋지 않아 걱정이라면? 'TEAS'에 주목하세요! (난소 반응 저하 환자에서의 침 치료...)

    3. **본문 구성 규칙:**
       - **핵심 구조:** 각 논문 설명은 반드시 **🔍 어떤 환자에게?**, **💊 어떤 치료를?**, **📊 결과는?** 의 3단 구성을 따르세요.
       - **용어 순화:** N수, P값 같은 통계 용어보다는 "총 00명을 대상으로", "통계적으로 유의미하게" 등으로 풀어서 설명하세요.
       - **용어 통일:** - Huzhang 등 외래어 기원 약재는 반드시 한국어 명칭(예: 호장근)을 찾아 괄호에 병기하세요. (모르면 해당 논문 제외)
         - EA, MA 등 약어는 최초 1회 Full Name(예: 전침, 수기침)을 풀어 써주세요.

    4. **강조 및 코멘트:**
       - 중요한 결과나 수치는 **굵게(Bold)** 처리하세요.
       - 각 논문 끝에는 **> 💡 닥터's 코멘트** 섹션을 인용구 문법(`>`)으로 만들어, 임상 활용 팁이나 독자에게 건네는 따뜻한 조언을 1~2문장으로 작성하세요.

    5. **단신 (Shorts) 처리:**
       - 약물 독성이나 부작용 관련 논문은 제목 앞에 ⚠️ 이모지를 붙여 주의를 환기하세요.

    [출력 포맷 (Markdown)]
    
    # 📅 {date_str} 한의 임상 브리핑

    (독자들에게 건네는 짧고 따뜻한 인사말 2~3문장)

    ---
    ### 🏆 Today's Pick: 집중 탐구
    *오늘 가장 주목해야 할 핵심 논문 2편입니다.*

    #### 1. [매력적인 블로그용 제목] ([논문 원제 번역])
    (이 논문이 왜 중요한지, 어떤 상황의 환자에게 필요한지 1~2문장의 도입부)

    * **🔍 어떤 환자에게?** (대상 환자 수와 특성 요약)
    * **💊 어떤 치료를?** (중재법, 비교군, 구체적 방법 요약)
    * **📊 결과는?** (핵심 결과, **수치 강조**, 효과 요약)

    > **💡 닥터's 코멘트**
    > (임상 적용 팁 또는 환자를 위한 조언, 따뜻하고 신뢰감 있는 어조)

    <br>

    #### 2. [매력적인 블로그용 제목] ([논문 원제 번역])
    (도입부)
    * **🔍 어떤 환자에게?** ...
    * **💊 어떤 치료를?** ...
    * **📊 결과는?** ...
    ...
    *바쁘신 분들을 위해 핵심만 빠르게 요약합니다!*

    1. **[논문 제목]**
       (내용 요약 1~2문장. **핵심 결과**는 굵게 표시)
       🔗 [Link](https://pubmed.ncbi.nlm.nih.gov/[PMID])
    
    2. **⚠️ [주의] [약물/독성 관련 제목]** (부작용 사례나 주의사항 요약)
       🔗 [Link](https://pubmed.ncbi.nlm.nih.gov/[PMID])
    ...
    **📢 Editor's Note**
    (마무리 인사 및 텔레그램/아카이브 홍보 멘트)
    
    * 📨 **텔레그램에서 편하게 받아보기:** [링크 클릭](https://t.me/+phvF8f8tfv41Njll)
    * 📚 **지난 아티클 모아보기:** [아카이브 바로가기](https://kmd-paper-curator.streamlit.app/)

    오늘의 포스팅이 유익하셨다면 **공감(❤️)과 이웃 추가** 부탁드립니다!
    
    #한의학논문 #한의사 #[핵심키워드1] #[핵심키워드2] ...
"""
            ...
            if st.button("🚀 심층 분석 & 글쓰기"):
                if not openai_api_key: st.error("Key 없음")
                    ...
                    with st.spinner("3. 심층 칼럼 작성 중..."):
                        final_prompt = f"""
    [Role & Persona]
    당신은 20년 임상 경력을 가진 베테랑 한의사이자, 어려운 의학 논문을 따뜻하고 알기 쉽게 풀어주는 '건강 멘토'입니다.
    단순한 번역이 아닙니다. 진료실에서 환자분들에게 다정하게 설명하듯, 전문성과 공감 능력을 갖춘 글을 작성하세요.

    [Input Data]
    - Situation: {status_msg}
    - Main Paper: {content_source[:25000]}
    - Evidence/Consensus: {evidence}

    [Writing Guidelines - Adaptive Structure]
    논문의 성격(효능 입증 vs 부작용 경고)과 중재법(침/약/추나)에 맞춰 아래 구조를 유연하게 적용하세요.

    1. **Title Selection (Hooking & Searchable)**
       - **가장 첫 줄**에 `[논문리뷰]` 말머리로 시작하는 제목을 작성하세요.
       - 제목은 논문 제목의 직역이 아니라, **타겟 독자(환자/보호자)의 '결핍'이나 '호기심'을 자극**해야 합니다.
       - **(효능 논문 예시):** [논문리뷰] 수술 권유받은 허리 디스크, '이 치료' 병행했더니 놀라운 결과가?
       - **(부작용 논문 예시):** [논문리뷰] 무릎에 좋다는 '우슬', 집에서 달여 드시나요? (간 수치 주의)

    2. **Structure & Formatting (엄격 준수)**
       제목 이후, 본문은 아래 5단 구성을 따르며 각 소제목 앞에 지정된 이모지를 사용하세요.

       **(1) 도입부 (에피소드):**
          - "진료실 문을 열고 들어오시는 환자분들 중..."과 같이 임상 현장의 에피소드로 시작하세요.
          - **(효능 논문):** 난치성 질환이나 만성 통증으로 고생하는 환자의 막막한 심정에 공감해주세요.
          - **(부작용 논문):** "몸에 좋다는 약초"를 시장에서 사 드시는 어르신들의 흔한 이야기를 꺼내세요.

       **(2) 🤔 문제 제기:**
          - 독자의 흥미를 확 끄는 질문을 던지세요.
          - 예: "이 치료법, 정말 과학적 근거가 있을까요?", "천연물은 무조건 안전할까요?"

       **(3) 📝 연구 결과/임상 증례 (스토리텔링):**
          - 딱딱한 요약 대신 **'사람의 이야기'**처럼 풀어주세요.
          - **(효능 연구):** "A치료를 받은 그룹은 통증 점수가 절반으로 뚝 떨어졌습니다."와 같이 *희망적인 변화*를 구체적 수치와 함께 강조하세요.
          - **(부작용 연구):** "어느 날 응급실을 찾게 된 70대 환자분의 이야기입니다."와 같이 *경각심*을 주는 서사로 작성하세요.

       **(4) 🌿 핵심 중재의 과학적 비밀 (전문가 설명):**
          - **[필수]** 제공된 논문 외에 'Network Pharmacology(네트워크 약리학)', '신경생리학적 기전(침/추나)', 또는 추가적인 과학적 근거를 1~2문장 덧붙여 전문성을 확보하세요.
          - **(약재):** 유효 성분과 작용 기전(항염증, 면역 조절 등)을 설명하세요.
          - **(침/추나):** 근육 이완, 신경 조절, 구조적 교정 등의 원리를 쉬운 비유(예: 고속도로 뚫리듯)로 설명하세요.

       **(5) 🧐 전문가의 비평 (Dr's Insight):**
          - **(효능 논문):** "이 연구는 XX 질환 치료의 새로운 희망을 보여줍니다."라며 긍정적 전망과 임상 적용 팁(치료 횟수 등)을 조언하세요.
          - **(부작용 논문):** "약재 자체가 나쁜 게 아닙니다. 정확한 진단 없는 '임의 복용'이 문제입니다."라며 전문가 진단의 중요성을 강조하세요.

       **(6) 📢 결론 (Action Plan):**
          - 독자가 당장 실천해야 할 행동 지침을 명확히 제시하고 따뜻하게 마무리하세요.
          - 예: "통증 참지 말고 내원하세요", "이상 증상이 생기면 즉시 복용을 멈추세요."

    3. **Tone & Manner:**
       - **신뢰(80%) + 다정함(20%):** 전문적인 내용을 다루지만 말투는 친절한 구어체("~했답니다", "~일까요?", "~해야 합니다")를 사용하세요.
       - **용어 현지화:**
         - 약재: Huzhang → 호장근, Psoraleae → 보골지 등 한국 한의학 명칭 사용.
         - 경혈: ST36 → 족삼리, LI4 → 합곡 등 한국 혈자리 이름 병기.

    4. **Constraints:**
       - Reference 리스트는 출력하지 마세요. (시스템이 처리함)
       - 이미지 삽입 지시문([Image of...])은 넣지 마세요.
       - 문단 사이에는 적절한 공백을 두어 가독성을 높이세요.

    [Goal]
    이 글을 읽은 독자가 "한의학 치료가 꽤 과학적이네?", "역시 아플 땐 한의원에 가서 제대로 물어봐야겠다"라고 느끼게 만드는 것이 목표입니다.
"""
